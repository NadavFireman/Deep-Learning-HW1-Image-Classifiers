...
def nn_forward_backward(
    ...
    # Numeric stability: subtract max from each row
    ...
    # L2 regularization on W1 and W2 (NOT on b1, b2)
    ...
    # Step 1: Gradient of loss w.r.t. scores (softmax gradient)
    dscores = probs.clone()
    dscores[torch.arange(N), y] -= 1
    dscores /= N
    
    # Step 2: Gradients of W2, b2 (second layer)
    grads['W2'] = h1.t().mm(dscores) + 2 * reg * W2
    grads['b2'] = dscores.sum(dim=0)
    
    # Step 3: Backprop through second layer to get dh1
    dh1 = dscores.mm(W2.t())
    
    # Step 4: Backprop through ReLU
    dh1[h1 <= 0] = 0
    
    # Step 5: Gradients of W1, b1 (first layer)
...
def nn_get_search_params():
    ...
    learning_rates = [0.95, 1.0, 1.05, 1.1]
    hidden_sizes = [650, 660, 670] 
    regularization_strengths = [1e-3, 1.5e-3, 2e-3] 
    learning_rate_decays = [0.975, 0.98, 0.985]
            ...
            for reg in regularization_strengths:
                for lr_decay in learning_rate_decays:
                    ...
                    stats = net.train(
                        ...
                        num_iters=11000, batch_size=800,
                        learning_rate=lr,
                    ...
                    # Print progress
                    print('hs=%d, lr=%.4f, reg=%.4f, decay=%.3f -> val_acc=%.4f' % (
                        hs, lr, reg, lr_decay, val_acc))
