...
def svm_get_search_params():
    ...
    learning_rates = [6.6e-3, 7.5e-3, 8.5e-3, 1e-2]
    regularization_strengths = [7e-5, 1e-4, 2e-4, 3e-4]
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        for k in range(num_classes):
            indicator = 1.0 if k == y[i] else 0.0
            dW[:, k] += (probs[k] - indicator) * X[i]
...
def softmax_get_search_params():
    ...
    learning_rates = [4e-2, 5e-2, 6e-2, 7e-2]
    regularization_strengths = [5e-6, 1e-5, 2e-5, 5e-5]
